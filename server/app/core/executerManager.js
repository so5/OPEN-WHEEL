 ...
 * @returns {boolean} -
 */
_internal.isExceededLimit = (JS, rt, outputText)=>{
 ...
 * @returns {string} -
 */
_internal.makeQueueOpt = (task, JS, queues)=>{
 ...
 */
_internal.decideFinishState = async (task)=>{
  let rt = false;
  try {
 ...
 * @returns {boolean} -
 */
_internal.needsRetry = async (task)=>{
...
async function register(task) {
  ...
  let executer;
  if (executers.has(_internal.getExecutersKey(task))) {
    ...
    executer = executers.get(_internal.getExecutersKey(task));
    const maxNumJob = _internal.getMaxNumJob(hostinfo);
    ...
    executers.set(_internal.getExecutersKey(task), executer);
  }
...
function cancel(task) {
  ...
  const executer = executers.get(_internal.getExecutersKey(task));
