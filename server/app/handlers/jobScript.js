...
export async function onAddJobScriptTemplate(socket, template) {
  await jobScriptTemplate.add(template);
  onGetJobScriptTemplates(socket);
}
export async function onUpdateJobScriptTemplate(socket, template) {
  await jobScriptTemplate.update(template);
  onGetJobScriptTemplates(socket);
}
export async function onRemoveJobScriptTemplate(socket, ID) {
  await jobScriptTemplate.remove(ID);
  onGetJobScriptTemplates(socket);
}
export async function onGetJobScriptTemplates(socket) {
